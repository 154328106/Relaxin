...
# Refuse to patch an unknown core: only the audited Relaxin 0.5.0 files are
...
# The common packager still verifies the untouched official engine and then
# swaps only UI-owned files. BaseBin mismatch is expected because the three
# Whitelist components and their regenerated trust cache are deliberate.
