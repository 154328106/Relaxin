...
HYBRID_049_OUTPUT ?= $(ROOT_DIR)/build/Artifacts/Relaxin-0.4.9-GroupedGlass-RootHide-TrueCore.tipa
UPSTREAM_049_APP ?=
HYBRID_050_PACKAGER := $(ROOT_DIR)/DevKit/Helpers/package-hybrid-050.sh
HYBRID_050_OUTPUT ?= $(ROOT_DIR)/build/Artifacts/Relaxin-0.5.0-GroupedGlass-RootHide-TrueCore.tipa
UPSTREAM_050_APP ?=
HYBRID_051_PACKAGER := $(ROOT_DIR)/DevKit/Helpers/package-hybrid-051.sh
HYBRID_051_OUTPUT ?= $(ROOT_DIR)/build/Artifacts/Relaxin-0.5.1-GroupedGlass-RootHide-TrueCore.tipa
UPSTREAM_051_APP ?=
HYBRID_052_PACKAGER := $(ROOT_DIR)/DevKit/Helpers/package-hybrid-052.sh
HYBRID_052_OUTPUT ?= $(ROOT_DIR)/build/Artifacts/Relaxin-0.5.2-GroupedGlass-RootHide-TrueCore.tipa
UPSTREAM_052_APP ?=
HYBRID_053_PACKAGER := $(ROOT_DIR)/DevKit/Helpers/package-hybrid-053.sh
HYBRID_053_OUTPUT ?= $(ROOT_DIR)/build/Artifacts/Relaxin-0.5.3-GroupedGlass-RootHide-TrueCore.tipa
UPSTREAM_053_APP ?=
HYBRID_054_PACKAGER := $(ROOT_DIR)/DevKit/Helpers/package-hybrid-054.sh
HYBRID_054_OUTPUT ?= $(ROOT_DIR)/build/Artifacts/Relaxin-0.5.4-GroupedGlass-RootHide-TrueCore.tipa
UPSTREAM_054_APP ?=
...
.PHONY: all help print-version \
        build build-ios lite-deb tipa ipa hybrid-048-tipa hybrid-049-tipa hybrid-050-tipa hybrid-051-tipa hybrid-052-tipa hybrid-053-tipa hybrid-054-tipa bootstrap-resources scan-license check test-host \
        kernel-offsets \
        format format-lint \
        clean \
...
help:
	...
	@echo "  hybrid-049-tipa       Put GroupedGlass over an audited upstream 0.4.9 core"
	@echo "  hybrid-050-tipa       Put GroupedGlass over an audited upstream 0.5.0 core"
	@echo "  hybrid-051-tipa       Put GroupedGlass over an audited upstream 0.5.1 core"
	@echo "  hybrid-052-tipa       Put GroupedGlass over an audited upstream 0.5.2 core"
	@echo "  hybrid-053-tipa       Put GroupedGlass over an audited upstream 0.5.3 core"
	@echo "  hybrid-054-tipa       Put GroupedGlass over an audited upstream 0.5.4 core"
...
hybrid-049-tipa: _check-tipa-tools build-ios
	@test -n "$(UPSTREAM_049_APP)" || { \
	    echo "error: set UPSTREAM_049_APP=/path/to/Payload/Relaxin.app" >&2; \
	    exit 64; \
	}
	/bin/bash "$(HYBRID_049_PACKAGER)" \
	    "$(APP_BUNDLE)" \
	    "$(UPSTREAM_049_APP)" \
	    "$(TIPA_ENTITLEMENTS)" \
	    "$(HYBRID_049_OUTPUT)"
...
hybrid-051-tipa: _check-tipa-tools build-ios
	@test -n "$(UPSTREAM_051_APP)" || { \
	    echo "error: set UPSTREAM_051_APP=/path/to/Payload/Relaxin.app" >&2; \
	    exit 64; \
	}
	/bin/bash "$(HYBRID_051_PACKAGER)" \
	    "$(APP_BUNDLE)" \
	    "$(UPSTREAM_051_APP)" \
	    "$(TIPA_ENTITLEMENTS)" \
	    "$(HYBRID_051_OUTPUT)"

hybrid-052-tipa: _check-tipa-tools build-ios
	@test -n "$(UPSTREAM_052_APP)" || { \
	    echo "error: set UPSTREAM_052_APP=/path/to/Payload/Relaxin.app" >&2; \
	    exit 64; \
	}
	/bin/bash "$(HYBRID_052_PACKAGER)" \
	    "$(APP_BUNDLE)" \
	    "$(UPSTREAM_052_APP)" \
	    "$(TIPA_ENTITLEMENTS)" \
	    "$(HYBRID_052_OUTPUT)"
...
hybrid-054-tipa: _check-tipa-tools build-ios
	@test -n "$(UPSTREAM_054_APP)" || { \
	    echo "error: set UPSTREAM_054_APP=/path/to/Payload/Relaxin.app" >&2; \
	    exit 64; \
	}
	/bin/bash "$(HYBRID_054_PACKAGER)" \
	    "$(APP_BUNDLE)" \
	    "$(UPSTREAM_054_APP)" \
	    "$(TIPA_ENTITLEMENTS)" \
	    "$(HYBRID_054_OUTPUT)"
