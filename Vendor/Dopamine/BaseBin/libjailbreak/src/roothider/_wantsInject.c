...
bool zqbb_wantsInject(const char *execName, const char *injectPath) {
    ...
    if (cache.xplist && cache.fileSize == s.st_size && cache.fileMtime == s.st_mtime) {
        return xpc_get_type(cache.xplist) == XPC_TYPE_DICTIONARY &&
               xpc_dictionary_get_bool(cache.xplist, execName);
    }
    ...
    if (cache.xplist)
        xpc_release(cache.xplist);
    cache.fileSize = s.st_size;
    cache.fileMtime = s.st_mtime;
    cache.xplist = xplist;

    return xpc_dictionary_get_bool(xplist, execName);
}
